...
var Invoices = [
  {
    ...
    prix_vente: 3.5,
    quantite_min: 50,
    date_arrivee: "01 Sep, 2025",
    date_peremption: "01 Sep, 2027",
    status: "Disponible",
    invoice_no: "24301901",
  },
  {
    ...
    prix_vente: 4.2,
    quantite_min: 30,
    date_arrivee: "15 Août, 2025",
    date_peremption: "15 Août, 2027",
    status: "Indisponible",
    invoice_no: "24301902",
  },
];
...
Array.from(Invoices).forEach(function (raw) {
  ...
  var tableRawData =
    ...
    "</td>\
        <td>" +
    raw.prix_achat.toFixed(2) +
    " €</td>\
        <td>" +
    raw.prix_vente.toFixed(2) +
    " €</td>\
        <td>" +
    raw.quantite_min +
    "</td>\
...
var perPage = 10;
var options = {
  valueNames: ["customer_name", "create_date", "due_date", "amount", "status"],
  page: perPage,
  pagination: true,
  plugins: [
    ListPagination({
    ...
    Swal.fire({
      ...
      if (result.value) {
        for (i = 0; i < ids_array.length; i++) {
          invoiceList.remove(
            "customer_name",
            `<a href="apps-invoices-overview.php">${ids_array[i]}</a>`
          );
        }
